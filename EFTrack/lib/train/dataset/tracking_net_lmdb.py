...
class TrackingNet_lmdb(BaseVideoDataset):
    ...
    def _read_bb_anno(self, seq_id):
        ...
        gt_str_list = decode_str(os.path.join(self.root),
                                 os.path.join("TRAIN_%d" % set_id, "anno", vid_name + ".txt")).split('\n')[:-1]
    ...
    def _get_frame(self, seq_id, frame_id):
        set_id = self.sequence_list[seq_id][0]
        vid_name = self.sequence_list[seq_id][1]
        return decode_img(os.path.join(self.root, "TRAIN_%d" % set_id),
