...
class VisionTransformer(torch.nn.Module):
    def __init__(self, search_size=384, template_size=192,
                 ...
                 window_size=[7, 7, 7],
                 kernels=[5, 5, 5, 5],
                 down_ops=[['subsample', 2], ['subsample', 2], ['']], norm_layer=nn.LayerNorm,
                 distillation=False, search_number=1, template_number=1,
                 drop_path_rate=0, use_checkpoint=False):
        super().__init__()
        ...
        # img_size = search_size + template_size
        resolution = search_size
        # Patch embedding
        ...
        #                                        Conv2d_BN(embed_dim[0] // 4, embed_dim[0] // 2, 3, 2, 1,
        #                                                  resolution=resolution // 4), torch.nn.ReLU(),
        #                                        Conv2d_BN(embed_dim[0] // 2, embed_dim[0]//2, 3, 2, 1,
        #                                                  resolution=resolution // 8))
        #####
        ...
        resolution = search_size*2 // patch_size
        ...
        for i, (stg, ed, kd, dpth, nh, ar, wd, do) in enumerate(
            ...
            if do[0] == 'subsample':
                ...
                #####resolution 不每次变成一半，始终16
                blk.append(torch.nn.Sequential(Residual(
                    Conv2d_BN(embed_dim[i + 1], embed_dim[i + 1], 3, 1, 1, groups=embed_dim[i + 1],
                              resolution=resolution_)),
                    Residual(
                        FFN(embed_dim[i + 1], int(embed_dim[i + 1] * 2), resolution_)), ))
                #######
    ...
    def forward_features(self, images_list):
        ...
        im_list = z_list + x_list
        im_feat1 = torch.cat(im_list[:2], dim=2)
        im_feat2 = torch.cat(im_list[-2:], dim=2)

        xz_feat = torch.cat([im_feat1, im_feat2], dim=3)


        # xz_feat = torch.cat([x_feat, z_feat], dim=1)
        # xz_feat:[2,128,16,16]
        # xz = self.pos_drop(xz_feat)  # xz=[2, 768, 768]
        ####################

        # xz_feat = torch.cat([x_feat, z_feat], dim=1)
        # B, C, _ = xz_feat.shape
        # xz_feat = xz_feat.reshape([B, C, 32, -1])



        x = self.blocks1(xz_feat)
        x = self.blocks2(x)
        x = self.blocks3(x)

        # xz = torch.nn.functional.adaptive_avg_pool2d(x, 1).flatten(1)
...
def vit_base_patch16(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        embed_dim=[192, 288, 384], depth=[1, 3, 4], num_heads=[3, 3, 4],
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    # pretrained = 'EfficientViT_m5'
    pretrained = None
...
def load_matching_weights(model, pretrained_weights):
    ...
    print('The length of the checkpoints is:', len(pretrained_weights))
    print('The length of the pretrained weights is:', len(pretrained_dict))
